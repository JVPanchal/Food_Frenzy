...
class BuggerScreen extends StatefulWidget {
  const BuggerScreen({super.key});

  @override
  State<BuggerScreen> createState() => _BuggerScreenState();
}

class _BuggerScreenState extends State<BuggerScreen> {
  @override
              ...
              child: Column(
                children: [
                  Row(
                    ...
                    children: [
                      Column(
                        children: List.generate(productlistburger.length, (index) {
                          return Padding(
                            padding: const EdgeInsets.all(8.0),
                            child: InkWell(
                              onTap: () {
                                setState(() {
                                  Navigator.of(context).pushNamed("detailburger");
                                  currentIndex = index;
                                });
                              },
                              child: productBox(
                                context, // Pass context to productBox
                                productlistburger[index]["img"],
                                productlistburger[index]["name"],
                                productlistburger[index]["price"],
                                productlistburger[index]["details"],
                              ),
                            ),
                          );
                        }),
